...
# Process files using null-separated output to handle paths with spaces
while IFS= read -r -d '' line; do
    # Extract status (first 2 chars) and filename (rest after space)
    status="${line:0:2}"
    file="${line:3}"
    
    # Only process modified files (status " M")
    if [ "$status" != " M" ]; then
        continue
    fi
