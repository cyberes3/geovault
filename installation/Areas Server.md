...
Each run applies replication diffs until the database sequence matches the planet daily server (so a large backlog is cleared in one go, which can take hours; once current, the next run finishes quickly). It will output something like this and then run through each day:
