...
from geo_lib.geocoding.ski_resorts import _search_nearby_ski_resorts
...
class ReverseGeocodingService:
    ...
    def _get_admin_and_protected_areas(self, latitude: float, longitude: float) -> Tuple[Dict[str, Optional[str]], List[Dict[str, str]]]:
        ...
        cached_admin = _GEOCODING_CACHE.get(admin_cache_key)
        cached_protected = _GEOCODING_CACHE.get(protected_cache_key)
...
"""
        ...
        if response:
            ...
            # Cache both results
            _GEOCODING_CACHE.set(admin_cache_key, admin_result, GEOCODING_CACHE_TTL)
            _GEOCODING_CACHE.set(protected_cache_key, protected_areas, GEOCODING_CACHE_TTL)
...
"""
        ...
        if response:
            ...
            _GEOCODING_CACHE.set(cache_key, cities, GEOCODING_CACHE_TTL)
        else:
...
"""
        ...
        if response:
            ...
            # Only cache on successful API response
            _GEOCODING_CACHE.set(cache_key, lakes, GEOCODING_CACHE_TTL)
        else:
            # Sort by distance even if no response (for consistency)
            lakes.sort(key=lambda x: x['distance_miles'])
    ...
    def _get_from_cache_or_fetch(
        ...
        _GEOCODING_CACHE.set(cache_key, tags, GEOCODING_CACHE_TTL)
    ...
    def get_location_tags(self, latitude: float, longitude: float) -> Tuple[List[str], List[GeocodingLogMessage]]:
        ...
        try:
            ...
            nearby_ski_resorts = _search_nearby_ski_resorts(latitude, longitude, 1.0)
            ski_tags = set()
