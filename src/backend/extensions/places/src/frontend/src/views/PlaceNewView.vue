    ...
    <div class="h-1/2 min-h-[200px] sm:h-auto sm:flex-1 sm:min-h-0 flex-shrink-0 relative border-b border-gray-300">
      ...
      <div class="absolute top-4 left-4 right-4 z-10 max-w-md">
        <div class="bg-white ring-1 ring-black/5 flex items-center p-1" :class="searchResults.length ? 'rounded-t-lg' : 'rounded-lg'">
          ...
          />
          <button type="button" @click="performSearch" class="p-2 hover:bg-gray-100 rounded-md text-gray-500 transition-colors">
        ...
        <div v-if="showResults && (searchResults.length > 0 || (searchQuery && !isSearching && searchTimeout === null))" class="bg-white ring-1 ring-black/5 max-h-60 overflow-y-auto w-full absolute top-full left-0 z-50 rounded-b-lg">
