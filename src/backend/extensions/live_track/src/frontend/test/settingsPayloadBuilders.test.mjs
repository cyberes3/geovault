...
test('buildTrackerSettingsPayloadFromSnapshot preserves every recent data filter option', () => {
  for (const option of ['all', '1min', '1h', '1d', '1w', '1m', 'session', 'current_session']) {
    ...
    assert.equal(payload.recent_data_window, option);
  }
});

test('buildTrackerSettingsPayloadFromSnapshot treats empty window as all', () => {
  const payload = buildTrackerSettingsPayloadFromSnapshot({
    name: 'Tracker',
    color: '#6C93DE',
    recentDataWindow: '',
  ...
  assert.equal(payload.recent_data_window, 'all');
});
