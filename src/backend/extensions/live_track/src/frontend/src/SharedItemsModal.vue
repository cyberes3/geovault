          ...
          <div class="flex items-start justify-between gap-2">
            ...
            <span
              :class="[
                ...
                isPublic(track) ? 'bg-amber-100 text-amber-800' : 'bg-blue-100 text-blue-800'
              ]"
            >
              {{ isPublic(track) ? 'Public' : 'Shared' }}
            </span>
          </div>
          <span
            class="mt-1.5 inline-flex items-center gap-1 rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-700"
            :title="isPublic(track) ? `${track.subscriber_count ?? 0} people have added it` : `Shared with ${(track.shared_with_emails || []).length} people`"
          >
            <svg
            ...
            <span>{{ isPublic(track) ? (track.subscriber_count ?? 0) : (track.shared_with_emails || []).length }}</span>
          </span>
        </button>
      </div>
    </div>
