...
export interface ArrowImageData {
  width: number;
  height: number;
  data: Uint8Array;
}

export function getArrowImageId(color: string | null | undefined, selected: boolean): string {
  const base = (color ?? '#6C93DE').replace('#', '');
  return 'track-arrow-' + (selected ? 'selected-' : '') + base;
}

export function getTrackArrowDataURL(color: string | null | undefined, selected: boolean): string {
 ...
 */
export function ensureArrowImage(mapInstance: MapLibreMap | null | undefined, color: string | null | undefined, selected: boolean = false): Promise<void> {
  if (!mapInstance?.getStyle()) return Promise.resolve();
  const id = getArrowImageId(color, selected);
  if (mapInstance.hasImage(id)) return Promise.resolve();
  return rasterizeArrowToImageData(color, selected).then((imageData) => {
    if (imageData && mapInstance.getStyle() && !mapInstance.hasImage(id)) {
      mapInstance.addImage(id, imageData, { pixelRatio: 1 });
    }
  });
}

export function rasterizeArrowToImageData(color: string | null | undefined, selected: boolean): Promise<ArrowImageData | null> {
