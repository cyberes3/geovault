...
fn main() -> Result<()> {
    ...
    if args.relation_tags_overwrite {
        ...
        let mut reader = osmio::arcpbf::PBFReader::new(BufReader::new(input_fp));
    ...
    // Use BufReader (not progress-bar wrapper): arcpbf needs exact Read semantics per blob.
    let input_fp = std::fs::File::open(&args.input_filename)?;
    let input_bar = progress_bars.add(
        ProgressBar::new(input_fp.metadata()?.len())
    ...
    let reader = osmio::arcpbf::PBFReader::new(BufReader::new(input_fp));
    ...
    let ways: Vec<_> = reader.ways().collect();
    ways.par_iter()
    ...
    if !ends_membership_filters.is_empty() || !args.ends_tag.is_empty() {
        ...
        let mut reader = osmio::arcpbf::PBFReader::new(reader);
        let ways: Vec<_> = reader.ways().collect();
        ways.par_iter()
            // ↑ all the ways
            //
            ...
            .filter(|(_, has_end_member_tags, has_end_point_tags)| {
...
fn do_read_nids_in_ne2_ways(
    mut reader: osmio::arcpbf::PBFReader<BufReader<File>>,
