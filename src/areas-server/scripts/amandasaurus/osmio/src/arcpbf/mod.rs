...
fn decode_nodes(
    ...
    for node in primitive_group.get_nodes() {
        let id = node.get_id() as ObjId;
        let raw_lat = node.get_lat();
        let raw_lon = node.get_lon();

        let scale_factor = granularity / COORD_PRECISION_NANOS;
        let mut internal_lat = (raw_lat as i32) * scale_factor;
        let mut internal_lon = (raw_lon as i32) * scale_factor;
        ...
        let tags: Option<Vec<(Arc<str>, Arc<str>)>> = if keys.is_empty() {
            None
        } else {
            Some(
                keys.iter()
                    .zip(vals.iter())
                    .filter_map(|(&k, &v)| {
                        let key = stringtable.get(k as usize).and_then(|x| x.clone());
                        let val = stringtable.get(v as usize).and_then(|x| x.clone());
                        match (key, val) {
        ...
        let timestamp = TimestampFormat::EpochNunber(info.get_timestamp());

        results.push(ArcOSMObj::Node(ArcNode {
            _id: id,
            _tags: tags,
            _lat_lon: Some((Lat(internal_lat), Lon(internal_lon))),
            _deleted: !info.get_visible(),
            _changeset_id: Some(info.get_changeset() as u32),
            _uid: Some(info.get_uid() as u32),
            _user: Some(
                stringtable[info.get_user_sid() as usize]
                    .clone()
                    .unwrap(),
            ),
            _version: Some(info.get_version() as u32),
