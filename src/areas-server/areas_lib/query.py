...
def query_single(
    ...
    """Run admin + protected + water + ocean + ski_resort (+ optional place) in parallel; return (admin_hierarchy, protected_areas, nearby_lakes, oceans, ski_resort)."""
    conn1 = pool.getconn()
    conn2 = pool.getconn()
    conn3 = pool.getconn()
    conn4 = pool.getconn()
    conn5 = pool.getconn()
    conn6 = pool.getconn() if city_radius_miles > 0 else None
    try:
        with ThreadPoolExecutor(max_workers=6 if conn6 else 5) as ex:
            ...
            f_ski_resort = ex.submit(lookup_ski_resort.run_ski_resort_single, conn5, lat, lon)
            f_place = ex.submit(lookup_places.run_place_single, conn6, lat, lon, city_radius_miles) if conn6 is not None else None
            futures = [f_admin, f_protected, f_water, f_ocean, f_ski_resort]
            ...
            ski_resort = f_ski_resort.result()
...
def query_batch(
    ...
    """Run admin + protected + water + ocean + ski_resort (+ optional place) batch; return list of (admin_hierarchy, protected_areas, nearby_lakes, oceans, ski_resort) in order."""
    if not points:
        return []
    n = len(points)
    ...
    try:
        with ThreadPoolExecutor(max_workers=6 if conn6 else 5) as ex:
            ...
            f_ski_resort = ex.submit(lookup_ski_resort.run_ski_resort_batch, conn5, indices, lons, lats)
            f_place = ex.submit(lookup_places.run_place_batch, conn6, indices, lons, lats, city_radius_miles) if conn6 is not None else None
            futures = [f_admin, f_protected, f_water, f_ocean, f_ski_resort]
            if f_place is not None:
                futures.append(f_place)
            wait(futures)
            ...
            ski_resort_by_idx = f_ski_resort.result()
    ...
    for i in range(n):
        ...
        ski_resort = ski_resort_by_idx.get(i)
...
def get_stats(conn: Any) -> Dict[str, Any]:
    ...
    ski_resort_stats = lookup_ski_resort.get_ski_resort_stats(conn)
    if ski_resort_stats is not None:
        out["ski_resorts"] = ski_resort_stats
    return out
