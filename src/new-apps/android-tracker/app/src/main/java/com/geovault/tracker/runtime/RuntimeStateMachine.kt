        ...
        return when (command.type) {
            ...
            RuntimeCommandType.RECOVER -> {
                if (!current.shouldBeRunning) {
                    ...
                    RuntimeDecision(
                        action = RuntimeAction(RuntimeActionType.NOOP, "recovery_skipped_service_running"),
                        nextState = current.copy(
                            lifecycleState = RuntimeLifecycleState.ACTIVE,
                            lastTransitionAtMs = now
                        )
            ...
            RuntimeCommandType.TASK_REMOVED -> {
                if (current.shouldBeRunning && !isServiceRunning) {
                    RuntimeDecision(
                    ...
                    RuntimeDecision(
                        action = RuntimeAction(RuntimeActionType.NOOP, "task_removed_noop"),
                        nextState = current.copy(lastTransitionAtMs = now)
                    )
                }
            }
