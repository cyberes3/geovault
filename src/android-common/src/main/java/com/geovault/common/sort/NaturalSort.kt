...
object NaturalSort {
    ...
    fun byName(locale: Locale): Comparator<String> = byName(locale) { it }

    fun <T> byName(locale: Locale, selector: (T) -> String): Comparator<T> =
        naturalOrderBy { selector(it).lowercase(locale) }
