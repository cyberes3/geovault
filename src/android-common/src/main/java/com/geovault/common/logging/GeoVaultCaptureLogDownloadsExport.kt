...
internal data class GeoVaultCaptureLogExportResult(
    val requestId: Long,
...
internal object GeoVaultCaptureLogDownloadsExport {
    ...
    fun export(context: Context, store: GeoVaultCaptureLogStore, requestId: Long): GeoVaultCaptureLogExportResult? {
        ...
        val displayName =
            GeoVaultCaptureLogFilename.buildExportDisplayName(appLabel, Instant.now(), compressed = true)
        val bounds = store.snapshotBounds()
        Log.i(
            TAG,
            "capture_export_start requestId=$requestId minId=${bounds.minId} maxId=${bounds.maxId} " +
        ...
        val uri =
            resolver.insert(collection, values)
                ?: run {
                    Log.e(TAG, "capture_export_failed requestId=$requestId error=mediastore_insert_null")
                    return null
                }
            ...
            resolver.openOutputStream(uri)?.use { raw ->
                ...
                GZIPOutputStream(counting).use { gzip ->
                    OutputStreamWriter(gzip, StandardCharsets.UTF_8).use { writer ->
                        writer.write("# GeoVault capture log export\n")
                        ...
                        val result = store.streamLogsAsText(
                            ...
                            if (nowMs - lastProgressAtMs >= PROGRESS_LOG_INTERVAL_MS) {
                                ...
                                Log.i(
                                    TAG,
                                    "capture_export_progress requestId=$requestId rows=${progress.rowsWritten} " +
                ...
                ?: run {
                    Log.e(TAG, "capture_export_failed requestId=$requestId error=open_output_stream_null")
                    resolver.delete(uri, null, null)
                    return null
                }
        } catch (e: Exception) {
            Log.e(TAG, "capture_export_failed requestId=$requestId error=write_failed", e)
            resolver.delete(uri, null, null)
            return null
        }
        ...
        Log.i(
            TAG,
            "capture_export_done requestId=$requestId displayName=$displayName rows=$rowsWritten " +
        ...
        return GeoVaultCaptureLogExportResult(
            requestId = requestId,
