...
import kotlin.random.Random
    ...
    internal fun PositioningRuntime.updateNotificationFromDb(broadcastStats: Boolean) {
    ...
    internal fun PositioningRuntime.broadcastSessionStats() {
    ...
    internal fun PositioningRuntime.updateRuntimeSnapshot(
    ...
    internal fun PositioningRuntime.applyAccuracyHoldUpdate(
    ...
    internal fun PositioningRuntime.buildAccuracyHoldLogKey(
        incomingAccuracyMeters: Float?,
    ...
    internal fun PositioningRuntime.accuracyMetersBucket(meters: Float?): Int {
        if (meters == null || !meters.isFinite() || meters < 0f) return -1
        return meters.toInt()
    }

    internal fun PositioningRuntime.logPointEmissionTroubleTransition(
    ...
    internal suspend fun PositioningRuntime.restoreLocalFreshnessFromDatabase(
        trackerId: String,
    ...
    internal fun PositioningRuntime.syncRuntimeStateStore(
    ...
    internal fun PositioningRuntime.maybeLogPositioningDiagnosticSnapshot(snapshot: TrackingRuntimeSnapshot) {
        val providerDecision = providerHealthController.evaluate(
            nowMs = System.currentTimeMillis(),
            isTracking = isTracking,
            expectsActiveFixDelivery = expectsActiveFixDelivery(),
    ...
    internal fun PositioningRuntime.transitionControlState(event: TrackingControlEvent, failureReason: String? = null) {
    ...
    internal fun PositioningRuntime.validateRuntimeInvariant(gpsProviderEnabled: Boolean) {
        when {
