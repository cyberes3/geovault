...
import kotlin.random.Random
    ...
    internal fun PositioningRuntime.enterStationaryRegion(
    ...
    internal fun PositioningRuntime.ensureGpsProviderReceiverRegistered() {
    ...
    internal fun PositioningRuntime.unregisterGpsProviderReceiverIfNeeded() {
        if (!gpsProviderReceiverRegistered) return
        runCatching { service.unregisterReceiver(gpsProviderReceiver) }
        gpsProviderReceiverRegistered = false
    }

    internal fun PositioningRuntime.enterWaitingForGpsProvider(reason: String) {
        if (
            !isTracking ||
    ...
    internal fun PositioningRuntime.resumeFromGpsProviderWait(reason: String) {
        if (
            !isTracking ||
            (gpsRuntimeState != GpsRuntimeState.WAITING_FOR_PROVIDER &&
                gpsRuntimeState != GpsRuntimeState.WAITING_FOR_PROVIDER_PAUSED)
    ...
    internal fun PositioningRuntime.pauseGps() {
        pauseGpsInternal(force = false)
    }

    internal fun PositioningRuntime.pauseGpsInternal(force: Boolean) {
    ...
    internal fun PositioningRuntime.startSensorWatchdog() {
        watchdogJob?.cancel()
    ...
    internal fun PositioningRuntime.requestStationaryFreshnessProbeIfDue(reason: String) {
        if (
            !isTracking ||
            (gpsRuntimeState != GpsRuntimeState.PAUSED_FOR_MOTION &&
                gpsRuntimeState != GpsRuntimeState.WAITING_FOR_PROVIDER_PAUSED)
    ...
    internal fun PositioningRuntime.resumeGps(reason: String = "significant_motion_resume") {
        if (
            !isTracking ||
            (gpsRuntimeState != GpsRuntimeState.PAUSED_FOR_MOTION &&
                gpsRuntimeState != GpsRuntimeState.WAITING_FOR_PROVIDER_PAUSED)
    ...
    internal fun PositioningRuntime.transitionGpsState(event: GpsRuntimeEvent, reason: String) {
