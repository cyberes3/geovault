...
class TrackerAccountViewModel(application: Application) : AndroidViewModel(application) {
    private val account = GeoVaultAccountController(
        scope = viewModelScope,
        appContext = application.applicationContext,
        authController = TrackerAppServices.from(application).initialAuthController(),
    )
