...
class SettingsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<View>(R.id.navMap).setOnClickListener { finish() }
        findViewById<View>(R.id.navSettings).setOnClickListener { /* already on Settings */ }
        ...
        findViewById<View>(R.id.headerLayout).setOnClickListener { finish() }
    ...
    private fun saveSelectedTracker(tracker: Tracker) {
        getSharedPreferences("geovault_prefs", Context.MODE_PRIVATE).edit()
            .putString("tracker_secret", tracker.tracker_secret ?: "")
            ...
            override fun onResponse(call: Call<Tracker>, response: Response<Tracker>) {
                runOnUiThread {
                    if (response.isSuccessful) {
                        ...
                        if (newTracker != null) {
                            saveSelectedTracker(newTracker)
            ...
            .setSingleChoiceItems(names, selectedIndex) { dialog, which ->
                ...
                scheduleTrackerOperationTimeout {
                    runOnUiThread {
                        ...
                        Toast.makeText(this@SettingsActivity, R.string.tracker_operation_timeout, Toast.LENGTH_SHORT).show()
                        val prefs = getSharedPreferences("geovault_prefs", Context.MODE_PRIVATE)
                        selectedTrackerText.text = prefs.getString("selected_tracker_name", null) ?: getString(R.string.select_tracker)
                    }
                }
                // Timeout applies: if fetch doesn't complete within 30s, loading is cleared and label reverted.
                // List API does not return tracker_secret; fetch full tracker so we can save the secret.
                TrackerRepository.getTracker(this, tracker.id) { full ->
                    runOnUiThread {
                        cancelTrackerOperationTimeout()
                        setTrackerOperationInProgress(false)
                        if (trackerOperationTimedOut) return@runOnUiThread
                        if (full != null) {
                            saveSelectedTracker(full)
                            TrackerRepository.clearCurrentTrackerCache()
                        } else {
                            Toast.makeText(this@SettingsActivity, R.string.could_not_save_tracker_selection, Toast.LENGTH_SHORT).show()
                            val prefs = getSharedPreferences("geovault_prefs", Context.MODE_PRIVATE)
                            selectedTrackerText.text = prefs.getString("selected_tracker_name", null) ?: getString(R.string.select_tracker)
                        }
                    }
