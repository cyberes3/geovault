...
internal class TrackingServiceDependencies(
    private val host: TrackingServiceHost,
    ...
    val lowAccuracyFallbackCoordinator = LowAccuracyFallbackCoordinator { host.currentPositioningRecoveryConfig() }
    val repeatedOutlierSuppressor = RepeatedOutlierSuppressor { host.currentPositioningRecoveryConfig() }
    ...
    fun wire(settingsRepository: TrackerSettingsRepository) {
        ...
        locationSessionCoordinator = LocationSessionCoordinator(service) { error ->
            ...
            host.scheduleLocationRequestReapplyRetry(reason = "async_registration_failure")
        }
        ...
        queueUploadEngine = QueueUploadEngine(
            ...
            pushContext = host.pushDispatcher,
            authenticatedClientProvider = { host.getAuthenticatedHttpClient() },
        )
        ...
        stationaryPingController = StationaryPingController(
            ...
            actions = object : StationaryPingActions {
                override fun requestProbe(reason: String) {
                    host.requestStationaryFreshnessProbe(reason = reason)
                }
        ...
        stationaryFreshnessCoordinator = StationaryFreshnessCoordinator(
            ...
            actions = object : StationaryFreshnessActions {
                ...
                override fun onProbeTimeout() {
                    host.pauseGpsInternal(force = true)
                }
            },
        )
    }
