...
import kotlin.random.Random
    ...
    internal fun PositioningRuntime.applyCurrentLocationRequest(reason: String): Boolean {
    ...
    internal fun PositioningRuntime.reapplyLocationRequestIfActive(reason: String) {
        if (
            !isTracking ||
            gpsRuntimeState == GpsRuntimeState.PAUSED_FOR_MOTION ||
            gpsRuntimeState == GpsRuntimeState.WAITING_FOR_PROVIDER_PAUSED
    ...
    internal fun PositioningRuntime.shouldDebounceLocationRequestReapply(reason: String): Boolean {
    ...
    internal fun PositioningRuntime.scheduleLocationRequestReapplyRetry(reason: String) {
    ...
    internal fun PositioningRuntime.startFixDeliveryWatchdog() {
        ...
        fixDeliveryWatchdogJob = serviceScope.launch {
            while (isTracking && runGeneration == trackingGeneration) {
                ...
                if (providerHealthController.shouldLog(decision)) {
                    ...
                    runtimeTelemetry.event(
                        eventName,
                        "$details gpsState=$gpsRuntimeState collectionPace=${state.collectionPace} lastAppliedAt=$lastLocationRequestAppliedAtMs",
                    )
                }
    ...
    internal fun PositioningRuntime.expectsActiveFixDelivery(): Boolean {
        return LocationRequestController.expectsActiveFixDelivery(isTracking, gpsRuntimeState)
    }

    internal fun PositioningRuntime.resolveLocationRequestFailureMessage(): String {
