...
import type { ToastManager } from '@/utils/toast'
...
export interface SetupCopyMapCoordinatesDeps {
 ...
 * @returns Teardown (removes the listener)
 */
export function setupCopyMapCoordinatesOnContextMenu(map: MapLibreMap, deps: SetupCopyMapCoordinatesDeps = {}): () => void {
  // `window.gv_core.GeoVault.toast` is declared as the narrower extension-facing `ToastService` type,
  // but the object assigned there at runtime (see `main.js`) is always the full `ToastManager` singleton.
  const toastApi: ToastManager | null =
    deps.toast ??
    (typeof window !== 'undefined' ? (window.gv_core.GeoVault.toast as ToastManager | undefined) : undefined) ??
    null
