...
class UploaderAccountViewModel(application: Application) : AndroidViewModel(application) {
    private val account = GeoVaultAccountController(
        scope = viewModelScope,
        appContext = application.applicationContext,
        authController = UploaderAppServices.from(application).initialAuthController(),
    )
