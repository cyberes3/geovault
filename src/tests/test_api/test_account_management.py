...
class TestEmailManagementAPI(TestCase):
    ...
    def test_email_change_valid(self):
        """Test changing email to valid new address."""
        change_data = {
        ...
        # Should succeed (may be 200 or 202 for pending verification)
        self.assertIn(response.status_code, [200, 202])

    def test_email_change_invalid_format(self):
        """Test changing email to invalid format."""
        change_data = {
    ...
    def test_email_change_already_exists(self):
        ...
        # Allauth AddEmailForm may allow duplicate emails across users
        # or reject them - depends on allauth configuration
        # Accept either behavior
        self.assertIn(response.status_code, [200, 400])

    def test_email_change_same_as_current(self):
        """Test changing email to same as current."""
        change_data = {
            'email': 'test@example.com'
        }
    ...
    def test_email_change_missing_new_email(self):
        """Test email change without email field."""
        response = self.client.post(
            '/api/user/email/change/',
            data=json.dumps({}),
            content_type='application/json'
    ...
    def test_email_change_empty_string(self):
        """Test email change with empty string."""
        change_data = {
            'email': ''
        }
    ...
    def test_email_change_whitespace_only(self):
        """Test email change with whitespace only."""
        change_data = {
            'email': '   '
        }
    ...
    def test_email_change_case_sensitivity(self):
        """Test email change with different case."""
        change_data = {
            'email': 'TEST@EXAMPLE.COM'
        }
        ...
        # May normalize case or treat as different email
        self.assertIn(response.status_code, [200, 202, 400])

    def test_email_change_with_plus_addressing(self):
        """Test email change with plus addressing (e.g., user+tag@example.com)."""
        change_data = {
        ...
        # Plus addressing should be valid (may be rejected by allauth validation)
        self.assertIn(response.status_code, [200, 202, 400])

    def test_email_change_international_domain(self):
        """Test email change with international domain."""
        change_data = {
