...
class TestCalTopoSecurity(TestCase):
    ...
    def test_feature_class_is_validated_whitelist(self):
        ...
        with patch('caltopo_extension.src.backend.views.single_import.get_feature', return_value=None):
            response = self.client.post('/api/extensions/caltopo-extension/import/feature/', {
                'map_id': 'map1',
                'feature_id': 'feature1',
                'feature_class': 'Marker'
    ...
    @patch('caltopo_extension.src.backend.utils.rate_limit.time.time')
