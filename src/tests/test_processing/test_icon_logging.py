...
def _valid_png_bytes() -> bytes:
    """A real, minimal decodable PNG. store_icon now re-encodes via Pillow, so tests
    exercising its storage/success paths need genuine image bytes, not just a magic header."""
    buf = BytesIO()
...
class TestIconLoggingBehavior:
    ...
    def test_store_icon_size_limit_logs_warning(self):
        ...
        with patch('geo_lib.processing.icons.storage.get_required_setting') as mock_get_setting:
    ...
    def test_store_icon_storage_failure_logs_error(self):
        ...
        icon_data = _valid_png_bytes()
        
        with patch('geo_lib.processing.icons.storage._get_icon_extension') as mock_ext:
            mock_ext.return_value = '.png'
            with patch('geo_lib.processing.icons.storage._get_storage_path') as mock_path:
    ...
    def test_store_icon_success_increments_stats(self):
        ...
        with patch('geo_lib.processing.icons.storage._get_icon_extension') as mock_ext:
            ...
            with patch('geo_lib.processing.icons.storage._get_storage_path') as mock_path:
                ...
                with patch('geo_lib.processing.icons.storage.hashlib') as mock_hashlib:
                    mock_hash = Mock()
...
class TestIconStatisticsTracking:
    ...
    def test_stats_multiple_icons_mixed_results(self):
        ...
        with patch('geo_lib.processing.icons.icon_manager.fetch_remote_icon') as mock_fetch:
            mock_fetch.return_value = _valid_png_bytes()
            with patch('geo_lib.processing.icons.storage._get_icon_extension') as mock_ext:
                mock_ext.return_value = '.png'
                with patch('geo_lib.processing.icons.storage._get_storage_path') as mock_path:
                    mock_storage = Mock()
    ...
    def test_log_levels_are_appropriate(self):
        ...
        with patch('geo_lib.processing.icons.storage._get_icon_extension') as mock_ext:
            ...
            with patch('geo_lib.processing.icons.storage._get_storage_path') as mock_path:
                ...
                mock_path.return_value = mock_storage
                store_icon(_valid_png_bytes(), 'test.png', import_log, stats)
