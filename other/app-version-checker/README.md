...
Cloudflare Worker that scans configured Gitea release repositories for GeoVault Android APKs, caches Gitea API responses, and exposes a small HTTP API plus a dashboard. Primarily used by the Android apps to check for new releases to avoid them having to perform the multi-HTTP call themselves.
...
| OPTIONS | `/check`, `/latest` | — | CORS preflight (same prefix rules). |
...
**Headers:** `Content-Type: application/json`. If `CHECK_SECRET` is configured, also `Authorization: Bearer <CHECK_SECRET>`.
...
- `appName` — required; must match the display name prefix in the APK filename (same as each app’s `EXPECTED_APP_NAME` in Android).
- `localFullCommitSha` — required; full 40-character lowercase hex SHA (`BuildConfig.GIT_COMMIT_SHA`).
- `releasesRepo` — optional; if omitted, the worker searches configured repos in order. If provided, must be exactly one of the configured release repos (`owner/repo`).
...
**Errors:** JSON body `{ "error": "<code>", "detail": "<message>" }` with appropriate HTTP status (e.g. `400` bad input, `404` no matching release asset, `401` bad bearer token, `502` upstream failure, `503` missing `GITEA_USER_AGENT`).
